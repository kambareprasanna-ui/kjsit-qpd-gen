...
Do not leave any slot blank.`;
    ...
    // Fix invalid unicode escapes sometimes generated by the AI
    content = content.replace(/\\u(?![\da-fA-F]{4})/g, "");
    // Postgres JSONB cannot store \u0000 (null bytes), so strip them to prevent "unsupported Unicode escape sequence" errors.
    ...
    const finalSets: GeneratedSet[] = diffList.map((diff) => {
      ...
      const alignedQuestions: GeneratedQuestion[] = pattern.map((slot, sIdx) => {
        ...
        // Strict AI Hallucination Filter
        if (matchedIdx !== -1 && bankQuestions.length > 0) {
          const rawQ = incomingQuestions[matchedIdx];
          const proposedText = String(rawQ.text || rawQ.question || "").trim();
          const normalizedProposed = proposedText.toLowerCase().replace(/\s+/g, " ");
          const isValidFromBank = bankQuestions.some(
            (bq) => bq.text.toLowerCase().replace(/\s+/g, " ") === normalizedProposed,
          );
          if (!isValidFromBank) {
            matchedIdx = -1;
          }
        ...
        // Graceful Downgrading (Ignoring Bloom)
        if (bankMatchIdx === -1) {
          bankMatchIdx = bankQuestions.findIndex((_, i) => !usedBankQuestionIndices.has(i));
        }
        // Re-using Questions over Fabrication
        if (bankMatchIdx === -1 && bankQuestions.length > 0) {
          bankMatchIdx = reuseCounter % bankQuestions.length;
          reuseCounter++;
        }
        ...
        // 6. Last resort if question bank text was empty or AI generation failed
        return {
          key: slot.key,
          text: `[Missing Question - Please regenerate]`,
          marks: slot.marks,
          bloom: slot.bloom,
          co: defaultCo,
